    ...
    <main>
        ...
        <div class="vld-parent">
            ...
            <div v-show="!isFetching" class="link-info">
                ...
                <p>Новая: <a :href="`https://${short}`" rel="nofollower norefferer" target="_blank">{{short}}</a></p>
                <p>Дата создания: <time :datetime="datetime">{{date}}</time></p>
                <p>Переходов по ссылке: {{clicks}}</p>
            </div>
        </div>
        <div class="actions">
            <button type="button" class="material-icons" title="назад к списку ссылок" @click="$router.push('/links')">arrow_back</button>
            <button type="button" class="material-icons" title="удалить ссылку" @click="linkDelete" style="color:red;">delete</button>
        </div>
    </main>
</template>
